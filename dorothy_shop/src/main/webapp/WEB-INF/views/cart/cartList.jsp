    ...
    <script>
      $(document).ready(function(){
        ...
        $("button[name='btnQtyChange']").on("click", function(){
          ...
          let btnQtyChange = $(this); // 변경 태그

          $.ajax({
            url : '/cart/cart_qty_change',
            data : {cart_code:$(this).parent().parent().find("input[name='cart_code']").val(), cart_amount:$(this).siblings("input[name='cart_amount']").val()},
        ...
        $("button[name='btnCartDel']").on("click", function(){
        	...
        	$.ajax({
        		...
        		data : {cart_code : $(this).parent().find("input[name='cart_code']").val()},
        		success : function(result) {
        			if(result == "success") {
        				alert("장바구니 항목이 삭제됨.");
        				current_tr.remove();
